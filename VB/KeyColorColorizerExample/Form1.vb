...
'            #Region "#BarSeries"
            ...
            Dim barSeries As New Series() With {.DataSource = LoadData(filepath), .Colorizer = CreateColorizer(), .ArgumentDataMember = "Country", .ColorDataMember = "Region", .View = New SideBySideBarSeriesView()}
...
'            #End Region ' #BarSeries
        ...
        Private Shared Function LoadData(ByVal filepath As String) As List(Of HpiPoint)
            ...
            For Each element As XElement In doc.Element("G20HPIs").Elements("CountryStatistics")
                points.Add(New HpiPoint() With {.Country = element.Element("Country").Value, .Product = Convert.ToDouble(element.Element("Product").Value), .Region = element.Element("Region").Value})
            Next element
            Return points
        End Function
        #End Region ' #DataLoad
